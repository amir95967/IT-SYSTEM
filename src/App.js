    ...
    <div className={`flex flex-col md:flex-row h-screen ${theme.bg} ${theme.text} font-sans overflow-hidden`} dir="rtl">
      ...
      <aside className={`hidden md:flex w-64 border-l ${theme.sidebar} ${theme.border} flex-col z-20 shadow-xl`}>
        ...
        <nav className="flex-1 px-4 space-y-1">
          ...
          {/* הגדרות - פתוח לכולם עם תווית שונה */}
...
function SettingsManager({ categories, subcategories, loadData, theme, tickets, announcement, setAnnouncement, isAdmin }) {
  // לקוח מתחיל בטאב אבטחה, אדמין מתחיל בקטגוריות
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6">
      <div className={`flex md:flex-col gap-2 p-2 rounded-2xl border h-fit ${theme.card} ${theme.border}`}>
          ...
          <button onClick={() => setActiveTab('security')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'security' ? 'bg-blue-600' : 'opacity-40'}`}>אבטחת חשבון</button>
          {isAdmin && <button onClick={() => setActiveTab('logs')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'logs' ? 'bg-slate-700' : 'opacity-40'}`}>לוגים</button>}
      </div>
      ...
      <div className="flex-1 text-right">
        {activeTab === 'security' && (
          <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border} max-w-sm shadow-xl`}>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-bold opacity-40 block mb-2 mr-1">סיסמה נוכחית</label>
                <input type="password" placeholder="הקלד סיסמה נוכחית" value={passData.old} onChange={e => setPassData({...passData, old: e.target.value})} className={`w-full p-4 rounded-xl text-sm ${theme.input} outline-none text-white`} />
              </div>
              <div>
                <label className="text-[10px] font-bold opacity-40 block mb-2 mr-1">סיסמה חדשה</label>
                <input type="password" placeholder="הקלד סיסמה חדשה" value={passData.next} onChange={e => setPassData({...passData, next: e.target.value})} className={`w-full p-4 rounded-xl text-sm ${theme.input} outline-none text-white`} />
              </div>
              <button onClick={handleUpdatePassword} disabled={passLoading} className={`w-full py-4 rounded-2xl font-black text-xs ${passLoading ? 'bg-slate-700' : 'bg-blue-600'} text-white transition-all`}>
                {passLoading ? 'מאמת נתונים...' : 'עדכן סיסמה עכשיו'}
              </button>
            </div>
          </div>
        )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                <div className="flex gap-2 mb-4">
                  ...
                  <button onClick={() => setIsPermanent(false)} className={`flex-1 py-2 rounded-lg text-xs font-bold ${!isPermanent ? 'bg-blue-600 text-white' : 'bg-white/5 opacity-50'}`}>מתוזמן</button>
                </div>
        ...
        {isAdmin && activeTab === 'categories' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border}`}>
                <h5 className="text-[10px] font-black opacity-50 mb-4">ניהול קטגוריות אב</h5>
        ...
        {isAdmin && activeTab === 'logs' && (
          <div className={`p-4 rounded-2xl border ${theme.card} ${theme.border} font-mono text-[10px] opacity-60 max-h-96 overflow-y-auto pr-2`}><h4 className="font-bold mb-4 flex items-center gap-2 justify-end text-blue-400"><Database size={14}/> מערכת לוגים</h4>{tickets.slice(0, 50).map(t => <div key={t.id} className="pb-1 border-b border-white/5 mb-1">[{formatJerusalemTime(t.created_at)}] פעולה על קריאה #{t.id.toString().substring(0,4)}</div>)}</div>
        )}
      </div>
    </div>
  );
...
// --- פונקציות תצוגה ומובייל ללא שינוי ---
