...
function SettingsManager({ categories, subcategories, loadData, theme, tickets, announcement, setAnnouncement }) {
  ...
  const saveAnnouncement = async (newData) => {
    ...
    if (error) alert("שגיאה"); else setAnnouncement(finalData);
    setAnnLoading(false);
  };
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      const { error: authError } = await supabase.auth.signInWithPassword({ email: user.email, password: passData.old });
      if (authError) throw new Error("סיסמה ישנה לא נכונה");
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6">
      <div className={`flex md:flex-col gap-2 p-2 rounded-2xl border h-fit ${theme.card} ${theme.border}`}>
         <button onClick={() => setActiveTab('categories')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'categories' ? 'bg-blue-600' : 'opacity-40'}`}>קטגוריות</button>
         <button onClick={() => setActiveTab('announcement')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'announcement' ? 'bg-amber-600' : 'opacity-40'}`}>הודעות</button>
         <button onClick={() => setActiveTab('security')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'security' ? 'bg-blue-600' : 'opacity-40'}`}>אבטחה</button>
         <button onClick={() => setActiveTab('logs')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'logs' ? 'bg-slate-700' : 'opacity-40'}`}>לוגים</button>
      </div>
          ...
          <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border} max-w-lg shadow-xl`}>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => saveAnnouncement({ ...announcement, active: !announcement.active })} className={`py-3 rounded-xl text-[10px] font-black uppercase ${announcement.active ? 'bg-green-600' : 'bg-slate-700 opacity-50'}`}>{announcement.active ? 'פעיל' : 'כבוי'}</button>
              ...
              <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                ...
                <div className="flex gap-2 mb-4">
                  <button onClick={() => setIsPermanent(true)} className={`flex-1 py-2 rounded-lg text-xs font-bold ${isPermanent ? 'bg-blue-600' : 'bg-white/5 opacity-50'}`}>לצמיתות</button>
                  <button onClick={() => setIsPermanent(false)} className={`flex-1 py-2 rounded-lg text-xs font-bold ${!isPermanent ? 'bg-blue-600' : 'bg-white/5 opacity-50'}`}>מתוזמן</button>
                </div>
              ...
              <button onClick={() => saveAnnouncement(announcement)} disabled={annLoading} className="w-full bg-amber-600 py-4 rounded-xl font-bold text-xs">עדכן הודעה</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'categories' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border}`}>
                <h5 className="text-[10px] font-black opacity-50 mb-4">קטגוריות</h5>
        ...
        {activeTab === 'security' && (
          <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border} max-w-sm`}>
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleUpdatePassword} disabled={passLoading} className={`w-full py-4 rounded-2xl font-black text-xs ${passLoading ? 'bg-slate-700' : 'bg-blue-600'} text-white`}>{passLoading ? 'מעדכן...' : 'שנה סיסמה'}</button>
            </div>
          </div>
        )}

        {activeTab === 'logs' && (
          <div className={`p-4 rounded-2xl border ${theme.card} ${theme.border} font-mono text-[10px] opacity-60 max-h-96 overflow-y-auto`}><h4 className="font-bold mb-4 flex items-center gap-2 justify-end text-blue-400"><Database size={14}/> LOGS</h4>{tickets.slice(0, 50).map(t => <div key={t.id} className="pb-1 border-b border-white/5 mb-1">[{formatJerusalemTime(t.created_at)}] Update ticket #{t.id.toString().substring(0,4)}</div>)}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-xl mx-auto"><div className={`p-6 md:p-8 rounded-3xl border ${theme.card} ${theme.border} shadow-2xl`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-right">
        <input required placeholder="נושא..." value={form.title} onChange={e => setForm({...form, title: e.target.value})} className={`w-full p-4 rounded-xl border ${theme.input} text-right text-white outline-none`} />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <textarea rows={4} placeholder="תיאור..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} className={`w-full p-4 rounded-xl border ${theme.input} text-right text-white outline-none`} />
        ...
        <button disabled={!token || isSubmitting} className={`w-full py-4 font-black rounded-2xl shadow-xl transition-all ${!token ? 'bg-slate-700' : 'bg-blue-600'} text-white`}>{isSubmitting ? 'שולח...' : 'שלח קריאה'}</button>
      </form>
    </div></div>
  );
}

// --- פונקציות תצוגה ללא שינוי ---
function NavItem({ icon, label, active, onClick }) { return <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${active ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-white/5'}`}>{icon} {label}</button>; }
function MobileNavItem({ icon, active, onClick }) { return <button onClick={onClick} className={`p-2 rounded-xl ${active ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>{icon}</button>; }
function StatCard({ label, value, icon, theme }) { return <div className={`p-5 rounded-2xl border ${theme.card} ${theme.border} text-right`}><div className="flex justify-between items-center mb-1"><span className="text-[10px] font-black text-slate-500 uppercase">{label}</span>{icon}</div><p className="text-2xl font-black">{value}</p></div>; }
