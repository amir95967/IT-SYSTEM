...
// פונקציית זמן ירושלים - הוספה ידנית של 3 שעות (UTC+3)
...
export default function App() {
  ...
  const fetchProfile = async (authUser) => {
    ...
    setUser({ id: authUser.id, email: authUser.email, name: data?.name || authUser.email.split('@')[0], role });
    await loadData();
    setLoading(false);
  };

  const loadData = async () => {
    const { data: ticketsData } = await supabase.from('tickets').select(`*, reporter:users!user_id(name, email), category:categories(name), subcategory:subcategories(name)`).order('created_at', { ascending: false });
    ...
    setAllUsers(u.data || []);
    setStats({ total: u.data?.length || 0, open: ticketsData?.filter(i => i.status !== 'closed').length || 0, closed: ticketsData?.filter(i => i.status === 'closed').length || 0 });
  };
    ...
    <div className={`flex h-screen ${theme.bg} ${theme.text} font-sans text-[14px]`} dir="rtl">
      ...
      <aside className={`w-56 border-l ${theme.sidebar} ${theme.border} flex flex-col z-20 shadow-xl`}>
        <div className="p-5 flex flex-col items-center gap-4">
          <img src={KALI_LOGO} alt="Kali Logo" className="w-28 cursor-pointer" onClick={() => window.location.hash = 'dashboard'}/>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className={`h-12 border-b ${theme.border}`}></header>
        <div className="flex-1 overflow-y-auto p-6">
          {view === 'dashboard' && <Dashboard stats={stats} tickets={tickets.filter(t => t.status !== 'closed')} onSelect={setSelectedTicket} theme={theme} />}
          ...
          {view === 'my_tickets' && <TicketsList title="הקריאות שלי" tickets={tickets.filter(t => t.user_id === user.id)} onSelect={setSelectedTicket} theme={theme} />}
          {view === 'archive' && <TicketsList title="ארכיון קריאות" tickets={tickets.filter(t => t.status === 'closed')} onSelect={setSelectedTicket} theme={theme} />}
          {view === 'users' && <UsersManager allUsers={allUsers} loadData={loadData} theme={theme} />}
          {view === 'settings' && <SettingsManager categories={categories} subcategories={subcategories} loadData={loadData} theme={theme} tickets={tickets} userEmail={user.email} />}
        </div>
      </main>
...
// --- Settings Manager (עם אפשרות הוספה ומחיקה מחדש) ---
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-500">
      ...
      <div className={`rounded-xl border ${theme.card} ${theme.border} overflow-hidden shadow-2xl`}>
        ...
        <TicketsTable tickets={tickets.slice(0, 15)} onSelect={onSelect} theme={theme} />
      </div>
    </div>
  );
}
...
// --- Tables & Lists ---
function TicketsTable({ tickets, onSelect, theme }) {
  return (
    <table className="w-full text-right text-[12px]">
      <thead className="opacity-40 font-black uppercase bg-black/20">
        <tr>
          ...
          <th className="px-5 py-2.5">ניהול</th>
        </tr>
      </thead>
      <tbody className={`divide-y ${theme.border}`}>
        {tickets.map(t => (
          <tr key={t.id} className="hover:bg-blue-500/10 transition-colors group">
            ...
            <td className="px-5 py-3"><button onClick={() => onSelect(t)} className="bg-blue-600 text-white px-3.5 py-1.5 rounded-lg font-bold text-[11px] cursor-pointer hover:bg-blue-500 shadow-md">נהל</button></td>
          </tr>
        ))}
      </tbody>
    </table>
...
function NavItem({ icon, label, active, onClick }) { return <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg font-bold text-[13px] transition-all cursor-pointer ${active ? 'bg-blue-600 text-white shadow-xl translate-x-1' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>{icon} {label}</button>; }
function StatCard({ label, value, icon, theme }) { return <div className={`p-5 rounded-xl border ${theme.card} ${theme.border} shadow-lg text-right`}><div className="flex justify-between items-center mb-1"><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{label}</span>{icon}</div><p className="text-2xl font-black">{value}</p></div>; }
...
function TicketsList({ title, tickets, onSelect, theme }) { return <div className="space-y-4 text-right"><h2 className="text-sm font-black italic border-r-4 border-blue-500 pr-3 opacity-80 uppercase tracking-tighter">{title}</h2><div className={`rounded-xl border overflow-hidden ${theme.card} ${theme.border} shadow-2xl`}><TicketsTable tickets={tickets} onSelect={onSelect} theme={theme} /></div></div>; }
