      ...
      <div className="flex-1">
        {activeTab === 'announcement' && (
          <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border} max-w-lg shadow-xl`}>
            <h4 className="font-bold text-amber-500 mb-6 flex items-center gap-2 justify-end"><Bell size={18}/> ניהול הודעות מתוזמנות</h4>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => saveAnnouncement({ ...announcement, active: !announcement.active })} className={`py-3 rounded-xl text-[10px] font-black uppercase ${announcement.active ? 'bg-green-600' : 'bg-slate-700 opacity-50'}`}>
                  {announcement.active ? 'מוצג כעת' : 'מוסתר'}
                </button>
                <select value={announcement.type || 'info'} onChange={(e) => setAnnouncement({...announcement, type: e.target.value})} className={`p-3 rounded-xl text-xs ${theme.input} outline-none`}>
                  <option value="info" className="bg-[#0A192F]">מידע (כחול)</option>
                  <option value="warning" className="bg-[#0A192F]">אזהרה (צהוב)</option>
                  <option value="critical" className="bg-[#0A192F]">קריטי (אדום)</option>
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="datetime-local" value={announcement.expiresAt || ''} onChange={(e) => setAnnouncement({...announcement, expiresAt: e.target.value})} className={`p-3 rounded-xl text-[11px] ${theme.input} outline-none`} />
                <input type="number" placeholder="מהירות גלילה" value={announcement.speed || 20} onChange={(e) => setAnnouncement({...announcement, speed: e.target.value})} className={`p-3 rounded-xl text-xs ${theme.input} outline-none`} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border}`}>
                ...
                <div className="flex gap-2 mb-4">
                  <input placeholder="חדשה..." value={newCat} onChange={e => setNewCat(e.target.value)} className={`flex-1 p-3 rounded-xl text-xs ${theme.input} outline-none`} />
                  <button onClick={async () => { if(newCat) { await supabase.from('categories').insert([{name: newCat}]); setNewCat(''); loadData(); }}} className="bg-blue-600 px-4 rounded-xl font-bold text-xs">הוסף</button>
                </div>
             ...
             <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border}`}>
                ...
                <select value={newSub.category_id} onChange={e => setNewSub({...newSub, category_id: e.target.value})} className={`w-full p-3 rounded-xl border text-xs ${theme.input} mb-3 outline-none`}>
                  <option value="" className="bg-[#0A192F]">בחר קטגוריית אב...</option>
                  {categories.map(c => <option key={c.id} value={c.id} className="bg-[#0A192F]">{c.name}</option>)}
                </select>
                <div className="flex gap-2">
                  <input placeholder="תת-קטגוריה..." value={newSub.name} onChange={e => setNewSub({...newSub, name: e.target.value})} className={`flex-1 p-3 rounded-xl text-xs ${theme.input} outline-none`} />
                  <button onClick={async () => { if(newSub.name && newSub.category_id) { await supabase.from('subcategories').insert([{name: newSub.name, category_id: newSub.category_id}]); setNewSub({name:'', category_id:''}); loadData(); }}} className="bg-emerald-600 px-4 rounded-xl font-bold text-xs">הוסף</button>
                </div>
        ...
        {activeTab === 'security' && (
          <div className={`p-6 rounded-2xl border ${theme.card} ${theme.border} max-w-sm`}>
            ...
            <input type="password" placeholder="סיסמה חדשה" value={passData.next} onChange={e => setPassData({...passData, next: e.target.value})} className={`w-full p-3 rounded-xl mb-4 text-xs ${theme.input} text-right text-white outline-none`} />
            <button onClick={async () => { const { error } = await supabase.auth.updateUser({ password: passData.next }); if (error) alert(error.message); else alert("הסיסמה שונתה!"); }} className="w-full bg-blue-600 py-3 rounded-xl font-bold text-xs text-white">עדכן סיסמה</button>
          </div>
        )}

        {activeTab === 'logs' && (
          <div className={`p-4 rounded-2xl border ${theme.card} ${theme.border} font-mono text-[10px] opacity-60 max-h-96 overflow-y-auto pr-2`}>
            <h4 className="font-bold mb-4 flex items-center gap-2 justify-end text-blue-400"><Database size={14}/> היסטוריית פעולות (LOG)</h4>
            {tickets.slice(0, 50).map(t => <div key={t.id} className="pb-1 border-b border-white/5 mb-1">[{formatJerusalemTime(t.created_at)}] LOG: קריאה #{t.id.toString().substring(0,6)} הועלתה</div>)}
          </div>
        )}
      </div>
    </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative">
            <select required className={`w-full p-4 rounded-xl border ${theme.input} bg-black/20 text-white outline-none text-right appearance-none cursor-pointer`} onChange={e => setForm({...form, category_id: e.target.value})}>
              <option value="" className="bg-[#0A192F]">בחר קטגוריה</option>
              {categories.map(c => <option key={c.id} value={c.id} className="bg-[#0A192F]">{c.name}</option>)}
            </select>
            <ChevronDown size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-400 pointer-events-none opacity-50" />
          </div>
          <div className="relative">
            <select required className={`w-full p-4 rounded-xl border ${theme.input} bg-black/20 text-white outline-none text-right appearance-none cursor-pointer`} onChange={e => setForm({...form, subcategory_id: e.target.value})}>
              <option value="" className="bg-[#0A192F]">תת-קטגוריה</option>
              {subcategories.filter(s => s.category_id === form.category_id).map(s => <option key={s.id} value={s.id} className="bg-[#0A192F]">{s.name}</option>)}
            </select>
            <ChevronDown size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-400 pointer-events-none opacity-50" />
          </div>
        </div>
...
function Login({ onLoginSuccess, theme }) {
  ...
  return (
    <div className="relative h-screen w-screen flex items-center justify-center bg-black overflow-hidden">
      <div className="absolute z-10 inset-0 bg-[#0B1E3B]/80 backdrop-blur-sm"></div>
      <div className="relative z-20 w-[85%] max-w-[380px] p-10 rounded-[3rem] border border-white/10 bg-[#0A192F]/60 backdrop-blur-xl flex flex-col items-center">
        ...
        <form onSubmit={handleLogin} className="w-full space-y-6">
          ...
          <button className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl shadow-xl hover:bg-blue-500 transition-colors">כניסה</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {tickets.length === 0 && <p className="opacity-30 italic text-xs">אין קריאות להצגה...</p>}
        {tickets.map(t => (
          <div key={t.id} onClick={() => onSelect(t)} className={`p-5 rounded-2xl border ${theme.card} ${theme.border} cursor-pointer hover:border-blue-500/50 transition-all group`}>
            ...
            <div className="flex justify-between items-center text-[11px] opacity-60"><span>{t.category?.name}</span>{t.solution && <span className="text-emerald-400 font-bold flex items-center gap-1"><CheckCircle2 size={12}/> פתורה</span>}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-end md:items-center justify-center z-[100] p-0 md:p-4 text-right" onClick={onClose}>
      <div className={`w-full md:max-w-md rounded-t-[2.5rem] md:rounded-[2.5rem] border p-8 shadow-2xl ${theme.card} ${theme.border}`} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6"><h3 className="text-[10px] font-black text-blue-500 uppercase tracking-widest">פרטי קריאה</h3><button onClick={onClose} className="text-blue-200/50 hover:text-white">✕</button></div>
        ...
        {isAdmin && ticket.status !== 'closed' && (
          <>
            <textarea value={solutionText} onChange={e => setSolutionText(e.target.value)} placeholder="כתוב פתרון ללקוח..." className={`w-full p-4 rounded-xl text-xs h-24 mb-4 ${theme.input} text-right text-white outline-none focus:ring-1 focus:ring-blue-500`} />
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => onUpdate('in_progress', solutionText)} className="bg-amber-600 hover:bg-amber-500 text-white py-4 rounded-2xl font-black text-xs transition-colors">בטיפול</button>
              <button onClick={() => onUpdate('closed', solutionText)} className="bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-2xl font-black text-xs transition-colors">סגור קריאה</button>
            </div>
          </>
        )}
      </div>
...
function UsersManager({ allUsers, loadData, theme }) {
  return (
    <div className="space-y-4">
       <h3 className="font-bold text-blue-500 uppercase text-[10px] tracking-widest border-r-4 border-blue-500 pr-3">ניהול עובדים</h3>
       <div className="grid gap-3">{allUsers.map(u => (<div key={u.id} className={`p-4 rounded-2xl border ${theme.card} ${theme.border} flex justify-between items-center group`}><div className="flex flex-col"><span className="font-bold group-hover:text-blue-400 transition-colors">{u.name}</span><span className="text-[11px] opacity-40">{u.email}</span></div><div className="flex items-center gap-3"><span className="px-2 py-0.5 bg-blue-500/10 rounded text-[10px] font-bold text-blue-400 uppercase">{u.role}</span><button onClick={async () => { if(window.confirm('מחק?')) { await supabase.from('users').delete().eq('id', u.id); loadData(); }}}><Trash2 size={16} className="text-red-500"/></button></div></div>))}</div>
    </div>
  );
}